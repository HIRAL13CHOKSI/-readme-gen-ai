...
## Overview
The VPC Subnet Calculator is a Python script designed to help users calculate subnets based on a specified VPC CIDR range and the number of Availability Zones (AZs) to utilize. It leverages the `netaddr` library for IP address manipulation and includes several utility functions for subnet calculations.
...
| Input                   | Output                  |
|-------------------------|-------------------------|
| VPC CIDR range          | List of calculated subnets |
| Number of Availability Zones | Subnet details         |
...
- **Issue**: Invalid CIDR range format.
  - **Solution**: Ensure the CIDR range is in the correct format (e.g., `10.0.0.0/16`).
